...
export default function BookingManagement() {
    ...
    const sortedTickets = useMemo(() => {
        return [...tickets].sort((a, b) => {
            const idA =
                parseInt((a.booking_ticket_id || "").replace(/\D/g, ""), 10) ||
                0;
            const idB =
                parseInt((b.booking_ticket_id || "").replace(/\D/g, ""), 10) ||
                0;
            return idA - idB;
        });
    }, [tickets]);
    ...
    const handleDeleteSelected = async () => {
        ...
        const confirmDelete = async () => {
            try {
                await axios.delete(
                    `${BASE_URL}/api/v1/booking_management/delete`,
                    {
                    ...
                    <div className={styles.container}>
                        <div className={styles.header}>
                            <h1 className={styles.title}>
                                ...
                                <span className={styles["ticket-count"]}>
                                    {tickets.length}
                                </span>
                            </h1>
                        ...
                        {loading ? (
                            ...
                            sortedTickets.map((ticket) => (
                                <BookingFlightTicket
                                    ...
                                    data={ticket}
                                    onEdit={() =>
                                        handleOpenUpdateTicketForm(ticket)
                                    }
                            ...
                            <table className={styles["modal-table"]}>
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>Giá tiền:</td>
                                        <td>
                                            {detailData.ticket_price?.toLocaleString(
                                                "vi-VN",
                                            )}{" "}
                                            VND
                                        </td>
